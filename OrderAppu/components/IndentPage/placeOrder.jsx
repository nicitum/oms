...
const PlaceOrderPage = ({ route }) => {
  ...
  // Fetch most recent order details (ported from Version 1)
  const fetchOrderDetails = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `http://${ipAddress}:8090/most-recent-order?customerId=${customerId}&orderType=${shift}`;
      ...
      if (!orderResponse.ok) {
        ...
        try {
          const productsResponse = await fetch(
            `http://${ipAddress}:8090/order-products?orderId=${orderData.order.id}`,
            {
              method: "GET",
              headers: {
                Authorization: `Bearer ${userAuthToken}`,
  ...
  }, [navigation, shift, calculateTotalAmount]);
  ...
  const checkCreditLimit = async () => {
    try {
      ...
      const creditLimitResponse = await fetch(
        `http://${ipAddress}:8090/credit-limit?customerId=${customerId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${userAuthToken}`,
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalBody}>
              ...
              <Text style={styles.modalOrderDetails}>Shift: {shift}</Text>
              <Text style={styles.modalOrderDetails}>

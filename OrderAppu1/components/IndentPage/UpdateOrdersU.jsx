...
const UpdateOrdersU = () => {
  ...
  const fetchUsersOrders = async (selectedDate = null) => {
    ...
    try {
      ...
      let url = `http://${ipAddress}:8091/get-orders/${custId}`
      // Only add date parameter if a valid date is provided
  ...
  const checkCreditLimit = async () => {
    try {
      ...
      const url = `http://${ipAddress}:8091/credit-limit?customerId=${customerId}`
      ...
      if (response.status === 200) {
        ...
        try {
          if (orderAmountDifference > 0) {
            await axios.post(
              `http://${ipAddress}:8091/credit-limit/deduct`,
          ...
          await axios.post(
            `http://${ipAddress}:8091/credit-limit/update-amount-due-on-order`,
            {
  ...
  const fetchLatestPriceFromOrderProducts = async (productId) => {
    try {
      ...
      const url = `http://${ipAddress}:8091/latest-product-price?productId=${productId}`
      const response = await axios.get(url, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const handleAddProductToOrder = async (productToAdd) => {
    ...
    try {
      ...
      const url = `http://${ipAddress}:8091/add-product-to-order`

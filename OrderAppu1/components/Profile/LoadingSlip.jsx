...
const LoadingSlipPage = () => {
    ...
    const fetchAdminOrders = useCallback(async () => {
        ...
        try {
            ...
            const nonCancelledOrders = ordersData.orders.filter(order => order.cancelled !== 'Yes' && order.approve_status === 'Accepted');
    ...
    return (
      <View style={[styles.dataRow, { backgroundColor: index % 2 === 0 ? '#FFFFFF' : '#F9FAFB' }]}>
        ...
        <Text
          ...
          accessibilityLabel={`Amount: ₹${orderForUser?.amount?.toFixed(2) || '0.00'}`}
        >
          ₹{orderForUser?.amount?.toFixed(2) || '0.00'}
        </Text>
        <Text
